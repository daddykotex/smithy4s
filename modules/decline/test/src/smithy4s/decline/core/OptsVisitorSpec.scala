...
object OptsVisitorSpec extends SimpleIOSuite {
  ...
  implicit class ParsesAsSyntax(expect: Expect) {
    ...
    def effectFailureSubstring(
      ...
      h => assert.same(List(expected), h.errors),
      fail("Expected failure, but got success 😩")
    )
  }
  ...
  pureTest("compile surjection - success") {
    val c = Length(min = Some(1))
    assert.parsed(
      parseOpts(
        sampleStruct(
          "surjection",
          string.addHints(c).validated(c)
        )
      )("a"),
      "a"
    )
  ...
  pureTest("compile surjection in list - success") {
    ...
    assert.parsed(
      parseOpts(
        sampleStruct(
          "surjection",
          list(string.addHints(c).validated(c))
        )
      )("a", "b", "c"),
      List("a", "b", "c")
    )
